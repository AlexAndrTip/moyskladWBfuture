<template>
  <div class="pagination-container" :class="{'top-pagination': isTop, 'bottom-pagination': !isTop}">
    <div class="pagination-controls">
      <button @click="emit('change-page', currentPage - 1)" :disabled="currentPage === 1 || selectedAllPages">Предыдущая</button>
      <span>Страница {{ currentPage }} из {{ totalPages }}</span>
      <button @click="emit('change-page', currentPage + 1)" :disabled="currentPage === totalPages || selectedAllPages">Следующая</button>
    </div>
    <div class="pagination-options">
      <label :for="`items-per-page-${isTop ? 'top' : 'bottom'}`">Товаров на странице:</label>
      <select :id="`items-per-page-${isTop ? 'top' : 'bottom'}`" :value="productsPerPage" @change="e => emit('update:productsPerPage', Number(e.target.value))">
