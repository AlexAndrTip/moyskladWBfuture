// src/views/TovaryPage/composables/useProductsOptimized.js
import { ref, watch, computed } from 'vue';
import axios from 'axios';

const API_BASE_URL = import.meta.env.VITE_API_BASE_URL;

export function useProductsOptimized(selectedIntegrationId, getToken, msFilter, complectFilter) {
  const products = ref([]);
  ...
  const getCacheKey = (page, searchTerm, msFilter, complectFilter, productsPerPage) => {
    return `${page}_${searchTerm || ''}_${msFilter || ''}_${complectFilter || ''}_${productsPerPage}`;
  };
  ...
  // Функция для загрузки всех товаров (для массовых действий)
  const fetchAllProducts = async () => {
    if (!selectedIntegrationId.value) return [];
    
    console.log('[API] Загружаем все товары для массовых действий...');
    ...
    try {
      ...
      if (response.data.success) {
        console.log(`[API] Загружено ${response.data.products.length} товаров для массовых действий`);
        return response.data.products;
      } else {
        console.error('[API] Ошибка при загрузке всех товаров:', response.data.message);
        return [];
      }
    } catch (error) {
      console.error('[API] Ошибка при загрузке всех товаров:', error);
      return [];
    }
  };

  // Функция для получения товаров с кэшированием
  const fetchProducts = async (page = currentPage.value, forceRefresh = false) => {
    ...
    const cacheKey = getCacheKey(page, searchTerm.value, msFilter.value, complectFilter.value, productsPerPage.value);
  ...
  // Функция для получения всех товаров по фильтрам (для массовых действий)
  const getAllProductsForBulkActions = async () => {
    console.log('[BULK] Получаем все товары для массовых действий...');
  ...
  watch([msFilter, complectFilter, productsPerPage], () => {
    clearCache();
  });
