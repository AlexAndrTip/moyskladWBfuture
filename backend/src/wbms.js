...
async function startWbPriceUpdates() {
  try {
    ...
    console.log('✅ Первое обновление цен завершено');
    
    // Устанавливаем периодическое обновление каждые 5 минут
    setInterval(async () => {
      try {
        console.log(`🕐 [${new Date().toISOString()}] Запуск планового обновления цен...`);
        await wbPriceService.getPricesForProducts();
        console.log('✅ Плановое обновление цен завершено');
      } catch (error) {
        console.error('❌ Ошибка при плановом обновлении цен:', error.message);
      }
    }, 5 * 60 * 1000); // 5 минут в миллисекундах
    
    console.log('🔄 Автоматическое обновление цен настроено каждые 5 минут');
    
  } catch (error) {
    console.error('❌ Ошибка при запуске автоматического обновления цен:', error.message);
    console.log('⚠️ Автоматическое обновление цен не запущено');
