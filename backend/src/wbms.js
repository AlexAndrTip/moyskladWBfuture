...
console.log('📦 Импортируем сервис автоматического обновления цен МойСклад...');
const msPriceAutoUpdateService = require('./services/msPriceAutoUpdateService'); // <-- сервис автоматического обновления цен МойСклад
console.log('✅ Импорт msPriceAutoUpdateService завершен');

// Создаем глобальный экземпляр сервиса автоматического обновления цен МойСклад
console.log('🔧 Создаем экземпляр MsPriceAutoUpdateService...');
const msAutoUpdateService = new msPriceAutoUpdateService();
console.log('✅ MsPriceAutoUpdateService создан успешно');
...
async function startMsPriceUpdates() {
  try {
    ...
    console.log('📊 Проверяем доступность сервиса...');
    
    if (!msAutoUpdateService) {
      throw new Error('Сервис автоматического обновления цен МойСклад не инициализирован');
    }
    
    console.log('✅ Сервис доступен, запускаем автоматическое обновление...');
...
app.listen(PORT, () => {
  ...
  console.log('⏰ Планируем запуск автоматического обновления цен МойСклад через 10 секунд...');
  setTimeout(() => {
    console.log('🕐 Время запуска автоматического обновления цен МойСклад наступило');
    startMsPriceUpdates();
